...
func SendCommand[T any](a *Agent, opts SendCommandOpts) ([]*SendCommandResponse[T], error) {
	if opts.Timeout == 0 {
	...
	go func() {
		...
		watcher, err := a.commandResponseBucket.Watch(cmd.Id)
	...
	_, err = a.commandWriter.Write(buffer.Bytes())
